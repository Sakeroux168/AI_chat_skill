...
description: Route an incoming request to the minimum necessary global skill; coordinator owns requirement discovery, coding agents do not automatically rerun it.
---
...
4. **Skill chaining is conditional, not ceremonial.** `grill → dispatch → implement → deliver → review` is possible, but phases are skipped when unnecessary. Do not mechanically force Architecture/Plan/subagent phases.
5. Project-specific skills/contracts take precedence over global guidance.
6. Before implementation, classify the **current change** by actual risk rather than repository importance, code volume, or prompt length.
7. When dispatching Codex, load `model-routing`, state Model + Reasoning, then use `agent-task-dispatch`. Prefer the lower reasonable effort and escalate on evidence.
8. Agent fan-out is not a default route. Use `agent-routing` only when separate ownership/parallelism/independent review is actually needed.
9. Discover professional capabilities by registry metadata; do not hard-code domain-specific skills here.
10. Load active `trusted` professional skills by default. `experimental` requires explicit user request/acceptance; never load `disabled`.
11. If no route fits, proceed with generic care: inspect, make the minimum change, test the direct risk, stop when done.
