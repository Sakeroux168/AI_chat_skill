...
它不是项目代码仓库，不保存产品代码，也不保存某个项目自己的长期记忆；这里只保存跨项目通用的聊天与 Agent 协作规则。
...
- 项目总控长期记忆与当前交接也属于项目本身，推荐放在 `<project>/.ai/coordinator/MEMORY.md` 和 `HANDOFF.md`；全局 Skill 只规定如何维护，不保存具体项目内容。
- 全局 Skill 放在本仓库，负责跨项目通用的协作方式、任务派发、模型路由、Review、PR 交付、总控连续性等规则。
